...
@pytest.mark.parametrize('filt', ['sinc_window', resamcupy.filters.sinc_window])
...
def test_filter_sinc(filt, window, num_zeros, precision, rolloff):
    ...
    interp, prec, _ = resamcupy.filters.get_filter(filt, **kwargs)
...
def test_filter_load():
    half_win, precision, _ = resamcupy.filters.get_filter('kaiser_best')


@pytest.mark.xfail(raises=NotImplementedError, strict=True)
def test_filter_missing():
    resamcupy.filters.get_filter('bad name')
